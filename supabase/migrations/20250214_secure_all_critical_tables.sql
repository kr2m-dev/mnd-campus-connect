-- Migration: Sécurisation de TOUTES les tables critiques avec RLS
-- Date: 2025-02-14
...
-- (nom, prénom, université uniquement - pas d'email, téléphone)
CREATE POLICY "Users can view public profile info"
ON public.profiles
FOR SELECT
TO authenticated
USING (true);  -- Accessible mais les infos sensibles doivent être masquées via la vue
...
COMMENT ON VIEW public.profiles_public IS 'Vue publique des profils - masque email, téléphone, avatar';
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_favorites_user_listing_unique
...
USING (
  sender_id = auth.uid()
  AND created_at > NOW() - INTERVAL '5 minutes'  -- Modifiable pendant 5 minutes
)
WITH CHECK (sender_id = auth.uid());
...
-- ============================================
-- VUES HELPER POUR FACILITER LES REQUÊTES
-- ============================================
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '================================================================';
  RAISE NOTICE '🔒 MIGRATION DE SÉCURITÉ RLS - TOUTES TABLES CRITIQUES';
  RAISE NOTICE '================================================================';
  RAISE NOTICE '';
